...
> **版本标记**：当前文档内容反映 `0.1.2` 完成态（含 `_b`：集成 ck/pk 内核 + 统一闸 + 智能交换中枢定位）。
...
你的应用 ──OpenAI/Anthropic──→ synth-loop（智能交换中枢）
                                  │
                                  ├─ 复杂度分类（规则 + 分形 LLM 判定）    ← 走哪条路径
                                  ├─ 分形路由（6 级 a-f）→ 选一条路径：
                                  │    简单对话 → chat / prompt_chat（策略注入）
                                  │    复杂任务 → task_chain / 相位推理（pk 多步）
                                  │
...
| 项目 | 独立做什么 | 在管道中的角色 |
|------|-----------|---------------|
| [text-cli](https://github.com/weihai-limh/text-cli) | 分布式指令执行——`AI:域;动作` 协议，10 个工具包随运行时附带，A0-A9 渐进式部署 | 工具/任务链执行引擎——路径 JSON → 指令执行 + `--async` 异步委托（相位执行走推理缝，非 tc） |
