// ========== build.js ==========
// 零依赖拼接：读 sl-web-chat-src/shell.html（外壳真源）→ 内联 sl-*.js 进 <script> 段 → 写制品
// 裁剪自 tc-web-chat：删 tc-cache/tc-parser/tc-approval/tc-quiet，加 sl-gate/sl-context（计划编辑 sl-plan 第二版）。
// 纪律：纯 node 内置 fs；不引入 webpack/rollup。
// 多语言：源为 i18n.json（不硬编码在 js 里）；构建时按 --lang 读取对应字段，拼 `const LANGS/I18N` 注入脚本段。
//   默认 both 产出 sl-web-chat.html（内嵌 zh+en）；--lang zh → sl-web-chat_zh.html；--lang en → sl-web-chat_en.html。
//   外壳 DOM 由 shell.html 维护，本脚本不做 DOM 注入。

const fs = require('fs');
const path = require('path');

const ROOT = __dirname;                       // sl-web-chat-src/ 所在目录
const WEB_ROOT = path.dirname(ROOT);          // sl-web-chat/
...
function main() {
    ...
    // 5. 启动入口：在末尾调 bootSlWebChat()
    const bootstrap = '\n// ---- bootstrap ----\nbootSlWebChat();\n';

    const newScript = head + '\n' + i18nSnippet + '\n' + parts.join('\n\n') + '\n' + bootstrap + '\n' + tail;

    // 6. 写制品（按 lang 决定文件名，下划线构型）
    const outName = (lang === 'both') ? 'sl-web-chat.html'
                  : (lang === 'zh')   ? 'sl-web-chat_zh.html'
                  :                     'sl-web-chat_en.html';
