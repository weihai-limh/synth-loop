"""管道 API (v0_1_2 T7, _a 3.6 降级为管理面)

9 端点: 创建/计划确认/启动/查询/暂停恢复中止/路径确认/审批/驳回/相位查询
_a: 降级为管理面——pipelines_api.enabled 开关（默认 false，9 端点 404）；
对外主契约走 chat 多轮驱动（pipeline-chat-protocol_zh.md）。
"""
...
def _api_enabled() -> bool:
    """pipelines_api.enabled 开关（默认 false——管理面关闭，对外主契约走 chat）"""
    config = get_config()
    return bool((config.get("pipelines_api") or {}).get("enabled", False))


async def _check_enabled() -> None:
    if not _api_enabled():
        raise HTTPException(status_code=404, detail="pipelines API disabled (chat 主契约驱动)")
...
async def create_pipeline(request: Request) -> JSONResponse:
    """创建管道（status=draft）——管理面：调试用；对外走 chat 相位意图"""
    await _check_enabled()
    try:
