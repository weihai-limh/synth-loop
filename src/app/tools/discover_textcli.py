"""discover_textcli 工具：发现 text-cli 可用指令（_a：走强化客户端）"""
...
async def handle_discover_textcli(session, args: dict) -> dict:
    ...
    try:
        ...
        directives = await client.discover(query if query else "json")
        discover_cache.set(cache_key, directives)
        return {"status": "success", "instructions": directives}
