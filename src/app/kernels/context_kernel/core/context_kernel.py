...
class ContextKernel:
    ...
    def infer(self, context_patch: Any) -> Any:
        ...
        sl 适配层封装（SlContextKernel 子类覆写），本基类保留 NotImplementedError
        ——vendored ck 纯副本原则：不改 ck 源码，sl 在外部扩展。
        """
        raise NotImplementedError("infer: 由 sl 适配层 SlContextKernel 覆写，本基类未实现")
