...
class PhaseDefinition:
    ...
    def __init__(self, index: int, name: str, description: str, mode: str = "single_ai",
        ...
        self.gates = gates or PhaseGates()
        self.status = status
        self.endpoint_alias = endpoint_alias  # _a: 本相位 path 提交的 tc 运行时（3.8 P4）
...
class PipelineSession:
    ...
    def __init__(self, intent: str, plan_config: PlanConfig,
        ...
        self.session_id = session_id  # _a: 挂 x-synthloop-session-id（chat 多轮驱动 + 随会话快照持久化）
        self.intent = intent
        ...
        self.phase_summaries: list[str] = []  # _a: 前序相位摘要（≤200字/条）——替代 accumulated_context
    ...
    def from_dict(cls, data: dict) -> "PipelineSession":
        ...
        session.phase_summaries = list(data.get("phase_summaries", []))
